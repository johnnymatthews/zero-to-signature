...
async fn run_command() -> anyhow::Result<String> {
    ...
    match cli.command {
        CliCommand::StoreProgram {
            ...
            let program = fs::read(format!("{}/target/wasm32-unknown-unknown/release/provide-a-password.wasm", get_project_root()?.to_string_lossy()))?.to_vec();
            generate_types();
            let config_interface = fs::read("provide-a-password_serialized_config_type.txt")?;
            let aux_data_interface = fs::read("provide-a-password_serialized_aux_data_type.txt")?;
